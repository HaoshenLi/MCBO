...
class FeedForward(nn.Module):
    def __init__(self, dim, out_dim, dropout=0.0):
        super().__init__()
...
class Block(nn.Module):
    """Transformer Block"""
    def __init__(self, dim, linear_dim, num_heads, dropout):
        super().__init__()
        self.attn = MultiHeadedSelfAttention(dim, dim, num_heads, dropout)
        self.attn_norm = nn.LayerNorm(dim)
        self.mlp = FeedForward(dim=dim, out_dim=linear_dim)
...
class SimpleTransformer(nn.Module):
    def __init__(self,
        ...
        self.attn_layer_lst = nn.ModuleList([
            Block(in_dim, linear_dim, num_head, dropout) for _ in range(num_attn)
        ])
    ...
    def forward(self, x, mask=None, pe=None):
        ...
        for attn_layer in self.attn_layer_lst:
            ...
            x = attn_layer(x, mask)
        if self.merge_token:
            return x[:, 0], self.attn_layer_lst[-1].attn.scores
        else:
            return torch.mean(x, dim=1), self.attn_layer_lst[-1].attn.scores